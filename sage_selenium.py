...
sage_selenium.py
════════════════════════════════════════════════════════════════════════════
Python/Celery wrapper for the Sage PUPPETEER scraper.

This file REPLACES the old ChromeDisplayMixin/Selenium sage_scraper.py.
It does NOT use Selenium, uc.Chrome, or ChromeDisplayMixin at all.
It launches: node sage_puppeteer.js --keyword ... --start-year ... --end-year ...

Logger name: SageScraper-{id}   (old Selenium version was "Sage-{id}")
════════════════════════════════════════════════════════════════════════════
"""
import glob
import logging
...
class SageScraper:
    """Puppeteer-based Sage scraper. Calls node sage_puppeteer.js."""

    _NODE_SCRIPT = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "sage_puppeteer.js"
    )
    ...
    def _setup_logger(self):
        # Logger name starts with "SageScraper-" (not "Sage-") to distinguish
        # from the old Selenium version in logs
        ...
        self.logger.info(f"SageScraper (Puppeteer) ==> Logger → {log_file}")
    ...
    def run(self):
        """Launch node sage_puppeteer.js and relay progress to Celery."""
        ...
        if not os.path.exists(self._NODE_SCRIPT):
            raise RuntimeError(
                f"sage_puppeteer.js not found at {self._NODE_SCRIPT}. "
                "Copy sage_puppeteer.js to /var/www/journal-scraper/"
            )
        ...
        self.logger.info(
            f"SageScraper ==> Launching Puppeteer: node sage_puppeteer.js "
        ...
        if proc.returncode != 0:
            raise RuntimeError(
                f"Sage Puppeteer scraper exited with code {proc.returncode}. "
                "Check logs/SageScraper-*.log for details."
            )
        ...
        self.logger.info(f"SageScraper ==> Done. Output: {final}")
