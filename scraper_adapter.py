...
#             # Nature now uses the standard SeleniumScraperWrapper path below
#             # (NatureScraper was migrated to ChromeDisplayMixin interface)
    ...
    def run_scraper(self, scraper_type: str, keyword: str, start_date: str,
        ...
        try:
            ...
            if registry_type == 'selenium':
                ...
                # Standardise the output location
                _conf_slug = conference_name.strip().replace(' ', '_') if conference_name and conference_name != 'default' else ''
                _conf_part = f"_{_conf_slug}" if _conf_slug else ''
                final_output = os.path.join(
                    self.output_dir,
                    f"{self.job_id}_{scraper_type}{_conf_part}_results.csv"
                )
                if output_file and os.path.exists(output_file):
                    if os.path.abspath(output_file) != os.path.abspath(final_output):
                        try:
